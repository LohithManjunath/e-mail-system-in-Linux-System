...
void get_content(GtkButton *SaveButton,GtkWidget *widget)
{
	...
	else
	{
		...
		char  info[50];
                ...
                char filename[100]={0};
		sendpost((char *)topic,(char *)kind,file_name,allrec,(char *)time,(char *)S_text,(char *)info);
		//---------------------
		create_succeed_window(1);
...
void  save_content(GtkButton *SaveButton, GtkWidget *widget)
{
        ...
        if(strcmp(rec,"") == 0)
	{
		...
		sendpost((char *)topic,(char *)kind,file_name,(char *)rec,time,(char *)S_text,(char *)info);
		//---------------------
		free(S_text);
		g_free(s);
		gtk_widget_destroy(widget);
