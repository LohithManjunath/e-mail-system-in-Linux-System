...
int opendrftb(char *uname, char *sortkey, void *p)
{
//open db
...
//select 
	sprintf(sql, "SELECT dmtitle,dmtime,no FROM draftbox_table WHERE uname = '%s'", uname);
...
//get 
	...
	{	
		if ((strcmp(sortkey, "lixingxing") == 0) || (strstr(azResult[i], sortkey) != NULL) || (strstr(azResult[i+1], sortkey) != NULL))
		{
			printf("i = %d\n",i);
			printf("Mail %d: %s %s %d\n", rm->mailnum, azResult[i], azResult[i+1], atoi(azResult[i+2]));
			strcpy(rm->title[rm->mailnum], azResult[i]);                    
			strcpy(rm->time[rm->mailnum], azResult[i+1]);                    
			rm->no[rm->mailnum] = atoi(azResult[i+2]);
			rm->mailnum++;
		}
	}
//close
...
void opendrtfb_fun(MSG msg,int temp_socket_descriptor)
{
	...
	switch( opendrftb(uname, msg.sortkey, &rm) )
	{
		case 1:
			printf("Draftbox is empty.\n");
			break;
		default:
			printf("correct\n");
...
printf("Mail %d: %s %s %d\n", i, rm.title[i], rm.time[i], rm.no[i]);
			}
