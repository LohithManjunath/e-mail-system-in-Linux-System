...
int openydmb(char *uname, char *sortkey, void *p)
{
//open db
...
//select 
	sprintf(sql, "SELECT rmtitle,rmtime,sender,no,rmstat FROM receivebox_table WHERE uname = '%s' AND rmstat = '%s'", uname, "y");
...
void openydmb_fun(MSG msg, int temp_socket_descriptor)
{
	...
	switch( openydmb(uname, msg.sortkey, &rm) )
	{
		case 1:
