 ...
 void *callback(void *fd)
{
  ...
  if(check[0]=='c')//receive text 
	{
	 ...
	 send(client_sockfd,add_name,200,0);
	 printf("%s\n",add_name);
	 send(client_sockfd,text_add,2048,0);
	}
  if(check[0]=='d')
	{	
		char _name[20] = {0}; 
  ...
  if(check[0]=='g')//receive preview information
	...
	if(check[0]=='s')//s:send
	{
		...
		recv(client_sockfd,file_name,20,0);
		printf("file_name:%s\n",file_name);
		char file_txt[2048]={0};
		int length = 0;
		while( length = recv(client_sockfd,file_txt,sizeof(file_txt),0))
		{
		   if(length < 0)
			{
...
//        int write_length = write(fp, buffer,length);    
		...
		mailin_db(myemail->topic,myemail->kind,myemail->sender,myemail->reciever,myemail->time,myemail->text,myemail->info,file_name,file_txt,rtnresult);//
	}
		free(fd);  
		close(client_sockfd);  
		 pthread_exit(NULL);  
