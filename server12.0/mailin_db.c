...
#include <string.h>
#include "mailin_db.h"
void mailin_db(char* name, char* kind, char* send, char* rece, char *time, char* text, char* info, char* add_name, char* text_add, char* rtnresult)
{       
      
	sqlite3 *db = NULL;
	int rc = 0;
	char *zErrMsg = 0;
	char db_sql_insert[8000] = {0};
...
//insert in text   table
	...
	sprintf(db_sql_insert, "INSERT INTO \"mail_text\" VALUES(%d,'%s','%s','%s','%s');",text_num, text, info, add_name,text_add);
