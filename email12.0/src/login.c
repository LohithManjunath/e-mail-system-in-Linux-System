...
#include "login.h"
#include "mail_win.h"
#define IP "10.10.28.234"
int total=0;
char name[20] = {0};
char text_add[2048]={0};
char add_name[200]="";
//char boxkind[20] = {0};
...
void on_fogetpsd_clicked(GtkWidget* widget,gpointer* data)
{	
	const char* entry_usrname_text;
	const char* entry_pwd_text;
	entry_usrname_text = gtk_entry_get_text(GTK_ENTRY(entry_usrname));
	...
	if(strlen(entry_usrname_text) > 0 )
	{
		g_printf("usrname: %s\n", entry_usrname_text);
		...
		g_printf("pwd: %s\n", entry_pwd_text);
                int flag=0;
		char rtnresult[25] ="changepsd";
		clientlogin(entry_usrname_text,rtnresult,&flag);//socket
		printf("flag ==%d",flag);
		printf("rtnresult in login.c : %s\n",rtnresult);
		if(flag == -3)
		create_error_window(5);
//change psd window
		else if (flag == -100)
		{
			show_changepsd(entry_usrname_text,rtnresult);
			//gtk_widget_destroy(widget);
...
int main(int argc, char* argv[])
{
	...
	button = gtk_button_new_with_label("Foget password?");
	g_signal_connect(G_OBJECT(button), "clicked", G_CALLBACK(on_fogetpsd_clicked), (gpointer*)NULL);
	gtk_box_pack_start(GTK_BOX(box), button, TRUE, TRUE, 0);
